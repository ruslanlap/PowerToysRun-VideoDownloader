...
'@
    ...
    private void TryTranscodeToH264(DateTime startedAt)
    {
        var candidates = new[] { "*.mp4", "*.mkv", "*.webm" }
            .SelectMany(p => Directory.GetFiles(_settings.DownloadPath, p))
            .Where(f => File.GetLastWriteTimeUtc(f) >= startedAt.AddSeconds(-5))
            .OrderByDescending(File.GetLastWriteTimeUtc);
        var file = candidates.FirstOrDefault();
        if (file == null) return;
        var codec = GetVideoCodec(file);
        if (codec is null or "h264") return;
        _context.API.ShowMsg("🔄 Transcoding to H.264", $"Adobe Premiere compatibility ({codec})", _iconPath);
        var tmpPath = Path.GetTempFileName() + ".mp4";
        try
        {
            var arguments = $"-y -hide_banner -nostdin -i \"{file}\" -map 0:v:0 -map 0:a:0? -c:v libx264 -preset fast -crf 18 -pix_fmt yuv420p -c:a aac -b:a 192k -movflags +faststart \"{tmpPath}\"";
            var (success, output, error) = RunYtDlpCommandWithFullOutput(arguments, showWindow: false);
            if (success)
            {
                File.Delete(file);
                File.Move(tmpPath, file);
                _context.API.ShowMsg("✅ Transcode complete", $"Successfully transcoded to H.264/AAC", _iconPath);
            }
            else
            {
                File.Delete(tmpPath);
                _context.API.ShowMsg("⚠️ Transcode failed", $"Failed to transcode to H.264. Original file kept.", _iconPath);
            }
        }
        catch (Exception ex)
        {
            if (File.Exists(tmpPath)) File.Delete(tmpPath);
            _context.API.ShowMsg("❌ Transcode error", ex.Message, _iconPath);
        }
    }

    private string GetVideoCodec(string filePath)
    {
        // Try ffprobe first
        var ffprobePath = Path.Combine(Path.GetDirectoryName(GetFfmpegExecutablePath()), "ffprobe.exe");
        if (File.Exists(ffprobePath))
        {
            var arguments = $"-hide_banner -print_format json -show_streams -select_streams v:0 \"{filePath}\"";
            var (success, output, error) = RunYtDlpCommandWithFullOutput(arguments, showWindow: false);
            if (success && !string.IsNullOrEmpty(output))
            {
                try
                {
                    ...
                    var codec = jsonDoc.RootElement.GetProperty("streams")[0].GetProperty("codec_name").GetString();
        ...
        var arguments = $"-hide_banner -i \"{filePath}\"";
        var (success, output, error) = RunYtDlpCommandWithFullOutput(arguments, showWindow: false);
        if (!string.IsNullOrEmpty(error))
        {
            var match = System.Text.RegularExpressions.Regex.Match(error, @"Video: (\w+)");
            if (match.Success)
            {
